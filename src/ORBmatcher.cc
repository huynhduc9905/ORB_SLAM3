...
namespace ORB_SLAM3
{
    ...
    vector<MatchCandidate> vMatches(vpMapPoints.size(), {nullptr, -1, -1, -1, -1});
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        ...
        if(!pMP || pMP->mnLastFrameSeen == F.mnId || (!pMP->mbTrackInView && !pMP->mbTrackInViewR) || pMP->isBad())
            continue;

        if(bFarPoints && pMP->mTrackDepth>thFarPoints)
            continue;

        MatchCandidate mc{pMP, -1, -1, -1, -1};
    ...
    for(size_t iMP=0; iMP<vpMapPoints.size(); iMP++)
    {
        const auto& mc = vMatches[iMP];
        if(!mc.pMP) continue;

        if(mc.bestIdx != -1 && !F.mvpMapPoints[mc.bestIdx])
        {
            F.mvpMapPoints[mc.bestIdx] = mc.pMP;
            nmatches++;
            if(mc.stereoIdx != -1 && !F.mvpMapPoints[mc.stereoIdx])
            {
                F.mvpMapPoints[mc.stereoIdx] = mc.pMP;
                nmatches++;
            }
        }

        if(mc.bestIdxRight != -1 && !F.mvpMapPoints[mc.bestIdxRight])
        {
            F.mvpMapPoints[mc.bestIdxRight] = mc.pMP;
            nmatches++;
            if(mc.stereoIdxRight != -1 && !F.mvpMapPoints[mc.stereoIdxRight])
            {
                F.mvpMapPoints[mc.stereoIdxRight] = mc.pMP;
                nmatches++;
            }
        }
    }
    ...
    int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
    {
        ...
        vector<MatchResult> vResults(LastFrame.N, {nullptr, -1, -1, -1, -1});
        ...
        // Sequential reduction of candidate results into CurrentFrame and rotHist
        for(size_t i=0; i<vResults.size(); i++)
        {
            const MatchResult &res = vResults[i];
            if(res.bestIdx != -1)
            {
                CurrentFrame.mvpMapPoints[res.bestIdx] = res.pMP;
                nmatches++;
                if(mbCheckOrientation && res.bin >= 0 && res.bin < HISTO_LENGTH)
                    rotHist[res.bin].push_back(res.bestIdx);
            }
            if(res.bestIdxRight != -1)
            {
                CurrentFrame.mvpMapPoints[res.bestIdxRight] = res.pMP;
                nmatches++;
                if(mbCheckOrientation && res.binRight >= 0 && res.binRight < HISTO_LENGTH)
                    rotHist[res.binRight].push_back(res.bestIdxRight);
            }
        }
